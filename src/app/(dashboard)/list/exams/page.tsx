...
type ExamItem = Exam & { lesson: {subject: {name: string, id: number}, class: {id: number, name: string}, teacher:{id: number, name: string}}};
...
const renderRow = (item: ExamItem) => (
    ...
    >
      ...
      <td className="hidden md:table-cell">
        <Link href={`/list/teachers/${item.lesson.teacher.id}`}>{item.lesson.teacher.name}</Link>
      </td>
      <td className="hidden md:table-cell">{new Intl.DateTimeFormat('fr-FR').format(item.startTime)}</td>
      <td>
...
const ExamListPage = async ({searchParams,}: {searchParams:{[key:string]: string} | undefined}) => {
  ...
  const p = page ? (parseInt(page) >=1 ? parseInt(page): 1) : 1;
    ...
    <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
          <TableSearch searchText={queryParams.search??''}/>
